...
const FlappyBird = () => {
  ...
  const restartGame = () => {
    if (canvasRef.current) {
      ...
      const restartGameLogic = async () => {
        ...
        game.reset(); // Сброс состояния игры
        game.start(); // Запуск новой игры
      };
  ...
  const goToMenu = () => {
    setIsMenuOpen(true);
    setIsGameOver(false); // Сбрасываем состояние окончания игры
  };
  ...
  }, [isMenuOpen]); // This ensures the game is initialized after the menu is closed
